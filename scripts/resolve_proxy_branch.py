...
while True:
    ...
    if response.status_code != 200:
        print(f"Status code from CI proxy for {request_url}: {response.status_code}", file=sys.stderr)
        sleep(3)
        continue
    ci_proxy_hash = response.text.strip()
    if ci_proxy_hash != densitydb_branch['commit']['sha']:
        print(f"HEADs not equal: origin={densitydb_branch['commit']['sha']} ci_proxy={ci_proxy_hash}", file=sys.stderr)
        sleep(3)
        continue
