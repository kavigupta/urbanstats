...
def check_site_size(directory):
    ...
    if loading_file_size > LOADING_FILE_THRESHOLD:
        print(
            f"\033[91mError: '{LOADING_FILE}' exceeds the threshold of {LOADING_FILE_THRESHOLD} bytes.\033[0m"
        )
        exit(1)
    ...
    print(
        f"Total size of '{directory}' (excluding '{LOADING_FILE}'): {total_size} bytes"
    )
    if total_size > OTHER_SIZE_THRESHOLD:
        print(
            f"\033[91mError: Total scripts size exceeds the threshold of {OTHER_SIZE_THRESHOLD} bytes.\033[0m"
        )
        exit(1)
