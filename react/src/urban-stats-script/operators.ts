...
function unaryOperator(op: string, unos: UnaryOperation[]): USSValue {
    return {
        ...
        value: defineFunction(`unaryOperator_${op}`, (ctx: Context, posArgs: USSRawValue[], _namedArgs: Record<string, USSRawValue>): USSRawValue => {
...
function binaryOperator(op: string, bos: BinaryOperation[]): USSValue {
    return {
        type: { type: 'function', posArgs: [{ type: 'anyPrimitive' }, { type: 'anyPrimitive' }], namedArgs: {}, returnType: { type: 'inferFromPrimitive' } },
        // eslint-disable-next-line @typescript-eslint/no-unused-vars -- needed for type signature
        value: defineFunction(`binaryOperator_${op}`, (ctx: Context, posArgs: USSRawValue[], _namedArgs: Record<string, USSRawValue>): USSRawValue => {
