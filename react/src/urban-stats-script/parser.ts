...
// Finds `-` unary operators and folds them into their constants, if possible
export function simplifyNegatives(n: UrbanStatsASTStatement | UrbanStatsASTExpression | UrbanStatsASTArg): void {
    switch (n.type) {
        case 'unnamed':
            simplifyNegatives(n.value)
            return
        case 'named':
            simplifyNegatives(n.value)
            return
        ...
        case 'identifier':
            return
        case 'attribute':
            simplifyNegatives(n.expr)
            return
        case 'call':
            simplifyNegatives(n.fn)
            n.args.forEach(simplifyNegatives)
            return
        case 'unaryOperator':
            simplifyNegatives(n.expr)
            if (n.operator.node === '-' && n.expr.type === 'constant' && n.expr.value.node.type === 'number') {
                n.expr.value.node.value *= -1
                // Replace this expr with its child
                Object.assign(n, n.expr)
            }
            return
        case 'binaryOperator':
            simplifyNegatives(n.left)
            simplifyNegatives(n.right)
            return
        case 'objectLiteral':
            n.properties.forEach(([, value]) => {
                simplifyNegatives(value)
            })
            return
        case 'vectorLiteral':
            n.elements.forEach(simplifyNegatives)
            return
        case 'assignment':
            simplifyNegatives(n.lhs)
            simplifyNegatives(n.value)
            return
        case 'expression':
            simplifyNegatives(n.value)
            return
        case 'statements':
            n.result.forEach(simplifyNegatives)
            return
        case 'if':
            simplifyNegatives(n.condition)
            simplifyNegatives(n.then)
            if (n.else) {
                simplifyNegatives(n.else)
            }
            return
        case 'do':
            n.statements.forEach(simplifyNegatives)
            return
        case 'condition':
            simplifyNegatives(n.condition)
            n.rest.forEach(simplifyNegatives)
            return
        case 'parseError':
            return
        case 'customNode':
