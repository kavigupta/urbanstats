...
export const defaultConstants: Constants = new Map<string, USSValue>([
    ...
    ...colorConstants,
    ...unitConstants,
    createNumberToNumberFunction('abs', Math.abs, 'abs', { kind: 'unchanged', takesAScalar: true }, 'Returns the absolute value of a number (removes the negative sign).'),
    ...
    createNumberToNumberFunction('nanTo0', (x: number) => isNaN(x) ? 0 : x, 'NaN to Zero', { kind: 'unchanged', takesAScalar: true }, 'Converts NaN values to 0, leaving other numbers unchanged.'),
    ['isNaN', {
    ...
    createVectorToNumberFunction('sum', values => values.reduce((a, b) => a + b, 0), 0, 'sum', { kind: 'unchanged', takesAScalar: true }, 'Returns the sum of all numbers in a vector.'),
