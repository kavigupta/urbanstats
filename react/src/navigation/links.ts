...
export function statistic_link(universe: string, statname: string, article_type: string, start: number, amount: number | string, order: string, highlight: string) {
    // make start % amount == 0
    if (amount != "All") {
        let amount_num: number = amount as number;
        start = start - 1;
        start = start - (start % amount_num);
        start = start + 1;
    }
    const params = new URLSearchParams()
