...
export function StatisticPanel(props: { 
    ...
    const background_color = (row_idx: number) => {
        if (row_idx > 0) {
    ...
    const style = (col_idx: number, row_idx: number) => {
            ...
            <div className="serif" ref={table_ref}>
                <div style={{ display: "flex" }}>
                    {column_names.map((name, i) => {
                        if (i === 0) {
                            return <div key={name} style={{ ...style(i, 0), "display": "flex", "justifyContent": "space-between", "flexDirection": "row" }}>
                                <div>{name}</div>
                ...
                {
                    index_range.map((i, row_idx) => <div key={i} style={{
...
function Pagination(props: { 
    ...
    const change_start = (curr_universe: string | undefined, new_start: number) => {
    ...
    const change_amount = (curr_universe: string | undefined, new_amount: "All" | string | number) => {
...
function ArticleLink(props: { longname: string }) {
    const curr_universe = useUniverse();
    return <a
...
function StatisticPanelSubhead(props: { article_type: string, rendered_order: string }) {
    const curr_universe = useUniverse();
...
function AscendingVsDescending({ on_click, is_ascending }: { on_click: (curr_universe: string | undefined) => void, is_ascending: boolean }) {
    const curr_universe = useUniverse();
