...
import { axisAndGrid, bottomLabelOffset, bottomLabelOffsetTranspose, computeDashPatterns, DetailedPlotSpec, groupedTipTitle, leftLabelOffset, leftLabelOffsetTranspose, paddedYDomain, SeriesPlot, transposeAwareTip } from './plots-general'
...
function xAxis(xidxs: number[], binSize: number, binMin: number, useImperial: boolean, transpose: boolean): [Plot.Markish[], (x: number) => string] {
    ...
    // pick the matching inward offset so the centered label clears the frame edge either way
    const labelOffset = transpose ? leftLabelOffsetTranspose : bottomLabelOffset
    return [
        [
