...
export function RampColormapSelector(props: { ramp: RampDescriptor, set_ramp: (newValue: RampDescriptor) => void, name?: string }) {
    ...
    const set_colormap = (colormap: EncodedColorMap) => {
        props.set_ramp({
            ...props.ramp,
            colormap: colormap,
        });
    ...
    const set_selected = (name: string) => {
        if (name === "Custom") {
    ...
    const set_custom_colormap = (custom_colormap: string) => {
        set_colormap({
            ...colormap,
            type: 'custom',
...
function SinglePointSelector({ value, color, cell, set_cell, remove_cell }: { value: number, color: string, cell: [number, string], set_cell: (newValue: [number, string]) => void, remove_cell: () => void }) {
    return <div
        style={{
            display: "flex",
...
function CustomColormapSelector(props: { colormap: string, set_colormap: (newValue: string) => void }) {
        ...
        function AddCellButton({ at_index }: { at_index: number }) {
            return <button
        ...
        const input_textbox = <input
            ...
            value={colormap_text}
            onChange={e =>
                props.set_colormap(e.target.value)
            }
        />
        return (
            <div>
