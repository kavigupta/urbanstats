...
function testsGeneric(
    ...
    async function aliceBobFriends(t: TestController, screenshots: boolean): Promise<JuxtastatUserState> {
        ...
        await t.expect(await friendsText(t)).eql([`You${bobPattern}`, 'Ask "Alice" to add youRemove'])
    ...
    test(`${props.name}-basic-friends-test`, async (t) => {
        ...
        await addFriend(t, 'Alice', '000000a')
        await t.expect(await friendsText(t)).eql([`You${charliePatternPrev}`, 'Ask "Alice" to add youRemove'])
        await restoreUser(t, 'Alice', state)
        ...
        await t.expect(await friendsText(t)).eql([`You${bobPattern}`, 'Ask "Alice2" to add youRemove'])
        await safeReload(t)
        await t.wait(1000)
        await t.expect(await friendsText(t)).eql([`You${bobPattern}`, 'Ask "Alice2" to add youRemove'])
    })
    ...
    test(`${props.name}-friends-bad-naming-test`, async (t) => {
        ...
        await addFriend(t, 'Charlie', '000000c')
        // Charlie added
        await t.expect(await friendsText(t)).eql([`You${alicePattern}`, `Bob${bobPattern}Remove`, 'Ask "Charlie" to add youRemove'])
