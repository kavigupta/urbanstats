...
async function runTest(test: string): Promise<TestResult> {
    let runner = testcafe[options.live ? 'createLiveModeRunner' : 'createRunner']()
        ...
        .browsers([`${options.browser} --window-size=1400,800 --hide-scrollbars --disable-search-engine-choice-screen --disable-skia-runtime-opts --disable-renderer-backgrounding --disable-features=LocalNetworkAccessChecks --use-gl=angle --use-angle=swiftshader --enable-webgl --ignore-gpu-blocklist`])
