...
export function runTests(platform: 'desktop' | 'mobile'): void {
    ...
    function frame(selector: string): Promise<Rect | undefined> {
        ...
        return ClientFunction(() => {
            ...
            return {
                x: Math.round(100 * (domRect.x - map0Rect.x) / map0Rect.width) / 100,
                y: Math.round(100 * (domRect.y - map0Rect.y) / map0Rect.height) / 100,
                width: Math.round(100 * domRect.width / map0Rect.width) / 100,
                height: Math.round(100 * domRect.height / map0Rect.height) / 100,
            }
    ...
    function mapPositionsEqual(a: MapPositions, b: MapPositions): boolean {
        ...
        return a.every((aMap, i) => {
            ...
            return (['x', 'y', 'width', 'height'] as const).every(key =>
                aMap.frame !== undefined && bMap.frame !== undefined
                && Math.abs(aMap.frame[key] - bMap.frame[key]) < frameThreshold,
            )
            && (['n', 'e', 's', 'w'] as const).every(key =>
                aMap.bounds !== undefined && bMap.bounds !== undefined
                && Math.abs(aMap.bounds[key] - bMap.bounds[key]) < boundsThreshold,
            )
        })
    }
    ...
    insetsEditTest(() => test, {
        ...
        after: [...defaultUSA.slice(0, 4), {
            frame: platform === 'mobile'
                ? { ...defaultUSA[4].frame, x: 0.56, y: 0 }
                : { ...defaultUSA[4].frame, x: 0.18, y: 0.35 },
            bounds: defaultUSA[4].bounds,
        }],
    ...
    insetsEditTest(() => test, {
        ...
        after: [{
            ...defaultUSA[0],
            bounds: platform === 'mobile'
                ? { n: 56, e: -74, s: 31, w: -139 }
                : { n: 52, e: -68, s: 26, w: -133 },
        }, ...defaultUSA.slice(1)],
        customInsetsAfterEdit: 1,
    })
