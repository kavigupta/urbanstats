...
import { categoryCheckbox, categoryToggleButton, enterEditMode, exitEditMode, filterBox, groupCheckbox, groupMemberRow, groupWarning, interactableGroupCheckbox, setCategoryExpanded, setSubcategoryExpanded, sourceCheckbox, subcategoryCheckbox, subcategoryToggleButton, yearCheckbox } from './edit_mode_test_utils'
...
export function articleEditTreeTest(platform: 'mobile' | 'desktop'): void {
    ...
    test('subcategory-expansion-persists', async (t) => {
        // Like a category's, the expansion is a setting rather than part of edit mode.
        await enterEditMode(t)
        await setCategoryExpanded(t, 'race', true)
        await setSubcategoryExpanded(t, 'race_composition', true)
        await safeReload(t)
        await enterEditMode(t)
        await t.expect(hispanicCheckInteractable.visible).eql(true)
