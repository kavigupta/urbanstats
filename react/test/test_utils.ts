...
export function comparison_page(locations: string[]) {
    const params = new URLSearchParams();
    params.set('longnames', JSON.stringify(locations));
    return TARGET + '/comparison.html?' + params.toString();
}

export async function check_textboxes(t: TestController, txts: string[]) {
...
export async function check_all_category_boxes(t: TestController) {
    const hamburgerMenu = Selector('div').withAttribute('class', 'hamburgermenu');
    if (await hamburgerMenu.exists) {
        await t.click(hamburgerMenu);
    }
...
async function prep_for_image(t: TestController) {
    await t.wait(1000);
    await t.eval(() => {
...
export async function screencap(t: TestController, name: string) {
...
export async function grab_download(t: TestController, name: string, button: Selector) {
    await prep_for_image(t);
    await t
        .click(button);
    await t.wait(3000);
...
export async function download_image(t: TestController, name: string) {
    const download = Selector('img').withAttribute('src', '/screenshot.png');
    await grab_download(t, name, download);
}

export async function download_histogram(t: TestController, name: string, nth: number) {
...
async function copy_most_recent_file(t: TestController, name: string) {
...
export async function download_or_check_string(t: TestController, string: string, name: string) {
