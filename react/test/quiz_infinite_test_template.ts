...
export function regressionTestForVersions(versions: number[], ignoredVersions: number[]): void {
    ...
    for (const v of vs) {
        if (!versions.includes(v) && !ignoredVersions.includes(v)) {
            throw new Error(`Version ${v} not in versions (${JSON.stringify(versions)}) or ignoredVersions (${JSON.stringify(ignoredVersions)}). This indicates that test harnesses need to be updated to add the newest quiz version.`)
        }
    }
