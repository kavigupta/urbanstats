...
test('under network limit home page navigate to california', async (t) => {
    ...
    await t.expect(await flushNetworkUsage('search')).lt(3_100_000)
    await t.pressKey('enter')
    await t.expect(await flushNetworkUsage('article')).lt(2_700_000)
})
...
test('under network limit stat counties by population', async (t) => {
    ...
    await waitForSelectedSearchResult(t)
    await t.expect(await flushNetworkUsage('search')).lt(3_700_000)
    await t.pressKey('enter')
