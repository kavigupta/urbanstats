...
test('maintain and restore scroll position back-forward', async (t) => {
    ...
    await t.expect(Selector('.headertext').withExactText('Texas').exists).ok() // Must wait for Texas to load, otherwise scrolling on the loading page is ineffective
    ...
    await t.click(Selector('a').withExactText('New York'))
    await t.expect(Selector('.headertext').withExactText('New York').exists).ok()
    await t.scroll(0, 400)
    await t.click(Selector('path[class*="Connecticut"]'))
    await t.expect(Selector('.headertext').withExactText('Connecticut').exists).ok()
    ...
    await t.expect(getScroll()).eql(100)
    await goBack()
    await t.expect(Selector('.headertext').withExactText('Texas').exists).ok()
