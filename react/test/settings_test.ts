...
test('check-settings-loaded', async (t) => {
    ...
    await t.expect(mapElement(/Huntington_Library/).exists).ok()
    // check that there's no element Pasadena_city or 91101
    await t.expect(mapElement(/Pasadena_city/).exists).notOk()
...
test('check-related-button-checkboxes-page-specific', async (t) => {
    ...
    await t.expect(mapElement(/San_Marino_city/).exists).ok()
    // neighborhoods should not be present (Huntington Library)
    await t.expect(mapElement(/Huntington_Library/).exists).notOk()
})

test('checkboxes-can-be-checked', async (t) => {
    // check that Pasadena CCD is not present
    await t.expect(mapElement(/Pasadena_CCD/).exists).notOk()
    ...
    await t.expect(mapElement(/Pasadena_CCD/).exists).ok()
    ...
    await t.expect(mapElement(/Briones_CCD/).exists).ok()
})
