...
void test('index mask', (): void => {
    ...
    // object with no vector fields
    const objWithNoVectorFields = undocValue(new Map([['a', 100], ['b', 200], ['c', 300]]), { type: 'object', properties: new Map([['a', { type: 'number' }], ['b', { type: 'number' }], ['c', { type: 'number' }]]) })
    assert.deepStrictEqual(
        indexMask(
            objWithNoVectorFields,
            undocValue([2, 2, 3], numVectorType),
            2,
        ),
...
void test('split mask testing', (): void => {
    ...
    const defaultLocInfo: LocInfo = { start: { block: { type: 'single', ident: 'test' }, lineIdx: 1, colIdx: 1, charIdx: 1 }, end: { block: { type: 'single', ident: 'test' }, lineIdx: 1, colIdx: 1, charIdx: 1 } }
    // basic mask testing
    assert.deepStrictEqual(
        splitMask(
            basicContext(),
