...
class CanadaElection:
    ...
    def read(self):
        zip_path = Path("named_region_shapefiles/CanadaGeneralElections") / "data" / self.folder_key / "processed" / "votes_by_poll_with_geometry.zip"
...
def aggregated_election_results_canada(shapefile):
    return aggregate_by_census_block_canada(2021, shapefile, election_results_by_block_canada())
...
def election_results_by_block_canada():
    ...
    blocks_edited = {}
    for election in tqdm.tqdm(canada_elections, desc="Disaggregating Canadian elections"):
